...
function wikiToMarkdown(wikitext,articleName,args) {
	if (!args) args = { db:'noDB', noSection:true, noTOC:true };
	if (!wikitext) return 'nothing to render';
	...
	html = html.replace( /^\.(.*)$/mg , '<div>$1</div>' );
	// links
	// html = html.replace( /\[\[([^\[\]\|#]*)(?:(\|[^\]\|#]*)+)?(?:#([^\]\|#]*))?\]\]/g , (entireMatch, articleName, displayName, anchor) => `[${displayName}]()` );
	html = html.replace( /\[\[([^\[\]\|#\n]*)((\|[^\]\|#\n]*)+)?(?:#([^\]\|#\n]*))?\]\]/g , (entireMatch, articleName, displayName, anchor) => `[${articleName}]()` );
	// html = html.replace( /\[([^\]\n ]*)(?: ([^\]\n]+))?\]/g , processExternalLink );
	...
	// return html;
	return { html:`<textarea style="min-width:40em;width:100%;min-height:60em">${html}</textarea>`, sidebarHtml:sidebarHtml, wikitext:wikitext, tocHtml:tocHtml };
	...
	function processTable(entireMatch,tableBody) {
		...
		// ***************** FINAL ******************
		entireMatch = entireMatch.replace( /¦TABLE¦([^¦]*)¦/g , "\n" );
		entireMatch = entireMatch.replace( /¦END TABLE¦/g , "\n" );

		entireMatch = entireMatch.replace( /¦ROW BOUNDARY¦/g , "" );

		entireMatch = entireMatch.replace( /¦TH¦([^¦]*)¦([^¦]*)¦END TH¦/g , "$2 | " );
		entireMatch = entireMatch.replace( /¦TD¦([^¦]*)¦([^¦]*)¦END TD¦/g , function(wholeMatch,m0,m1,m2) {
			// return '<td '+(m0||'')+'>\n'+(m1||'')+'\n</td>';
			return `${m1} | `;
		});
		entireMatch = entireMatch.replace( / \| $/gm, '' );
		entireMatch = entireMatch.replace( /\n\n/gm, '\n' );
...
exports.wikiToMarkdown = wikiToMarkdown;
